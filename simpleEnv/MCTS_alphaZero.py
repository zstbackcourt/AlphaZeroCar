...
class TreeNode(object):
    ...
    def get_value(self, c_puct):
        ...
        self._u = (c_puct * self._P *
                   np.sqrt(self._parent._n_visits) / (1 + self._n_visits)) / 10.0
...
class MCTS(object):
    ...
    def __init__(self, env, policy_value_fn, c_puct=5, n_playout=10000):
        ...
        # 在根节点展开
        action_probs, leaf_value = self._policy_value_fn(env.acts, self._root._ob.reshape(-1, self.env.ob_dim))
    ...
    def get_action(self, temp=1e-3, Dirichlet_coef=0.1):
        ...
        if self._is_selfplay:
            ...
            action = np.random.choice(
                acts,
                p=0.90 * probs + Dirichlet_coef * np.random.dirichlet(0.3 * np.ones(len(probs)))
            )

...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        Args:
            input_dims (dict of ints): dimensions for the observation (o), the goal (g), and the
                actions (u)（一个整型的字典，用来存储state，goal和action的维度）

            buffer_size (int): number of transitions that are stored in the replay buffer
                               存储在replay buffer中的transitions数
            hidden (int): number of units in the hidden layers
            layers (int): number of hidden layers
            network_class (str): the network class that should be used (e.g. 'baselines.her.ActorCritic')
            polyak (float): coefficient for Polyak-averaging of the target network
            batch_size (int): batch size for training
            Q_lr (float): learning rate for the Q (critic) network critic网络的学习率
            pi_lr (float): learning rate for the pi (actor) network actor网络的学习率
            norm_eps (float): a small value used in the normalizer to avoid numerical instabilities 防止nornalizer的数值不稳定
            norm_clip (float): normalized inputs are clipped to be in [-norm_clip, norm_clip]
            max_u (float): maximum action magnitude, i.e. actions are in [-max_u, max_u]
            action_l2 (float): coefficient for L2 penalty on the actions
            clip_obs (float): clip observations before normalization to be in [-clip_obs, clip_obs]
            scope (str): the scope used for the TensorFlow graph
            T (int): the time horizon for rollouts rollouts的时间范围
            rollout_batch_size (int): number of parallel rollouts per DDPG agent
            subtract_goals (function): function that subtracts goals from each other
            relative_goals (boolean): whether or not relative goals should be fed into the network
            clip_pos_returns (boolean): whether or not positive returns should be clipped
            clip_return (float): clip returns to be in [-clip_return, clip_return]
            sample_transitions (function) function that samples from the replay buffer 从Buffer中采样
            ...
            aux_loss_weight: Weight corresponding to the auxilliary loss also called the cloning loss 即多进程多GPU训练时的sum loss
        """
        ...
        self.dimo = self.input_dims['o'] # state的维度
        self.dimg = self.input_dims['g'] # goal的维度
        self.dimu = self.input_dims['u'] # action的维度

        # Prepare staging area for feeding data to the model.
        # 准备中转区域以将数据输送到模型。
        stage_shapes = OrderedDict() # 定义stage_shapes为一个有序类字典
        for key in sorted(self.input_dims.keys()):
            if key.startswith('info_'): # 检查key是否以字符串"info_"开头
                continue
        ...
        # 初始化replay buffer
        buffer_shapes = {key: (self.T-1 if key != 'o' else self.T, *input_shapes[key]) # *可以将元素拆成单个
    ...
    def _random_action(self, n):
        # 生成符合动作范围的随机动作，shape=[n,dimu]
        return np.random.uniform(low=-self.max_u, high=self.max_u, size=(n, self.dimu))

    def _preprocess_og(self, o, ag, g):
        # 对observation和goal进行预处理，实际上就是clip一下
    ...
    def step(self, obs):
        # 根据state，达到的goal和期望goal选择actions
    ...
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
        ...
        if compute_Q:
            vals += [policy.Q_pi_tf]
        # feed 将observation goal action送入网络
        feed = {
        ...
        # 对从网络中获得的action做后期处理
        u = ret[0]
        noise = noise_eps * self.max_u * np.random.randn(*u.shape)  # gaussian noise 生成高斯噪声
        u += noise
        u = np.clip(u, -self.max_u, self.max_u)
        # np.random.binomial(n,p,size)二项分布
        # n是试验次数
        # p是实验成功的概率
        # 每个样本返回的的是n次实验中事件发生的次数

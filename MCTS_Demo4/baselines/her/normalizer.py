...
class Normalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf, sess=None):
        ...
        a standard Normal distribution (i.e. have mean zero and variance one).
            确保observations近似于一个标准正太分布
        Args:
            size (int): the size of the observation to be normalized
            eps (float): a small constant that avoids underflows下溢
            default_clip_range (float): normalized observations are clipped to be in
                [-default_clip_range, default_clip_range]
            sess (object): the TensorFlow session to be used
        """
        self.size = size
        self.eps = eps # 防止发生下溢出的一个常量
        self.default_clip_range = default_clip_range
        self.sess = sess if sess is not None else tf.get_default_session()

        self.local_sum = np.zeros(self.size, np.float32)
        self.local_sumsq = np.zeros(self.size, np.float32) # 平方和
        ...
        self.update_op = tf.group(
            # tf.assign_add(ref,value,use_locking=None,name=None) 更新ref，方式ref=ref+value

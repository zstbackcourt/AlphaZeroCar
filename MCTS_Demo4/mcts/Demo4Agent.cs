...
public class Demo4Agent : Agent
{
    ...
    private Vector3 car_original_pos = new Vector3(40f, 0.0190f, 10.5f); // 车的初始位置
    private float car_original_eulerAngles_y = 0.0f;// 车的初始欧拉角
    ...
    private float complete_dis = 0.5f; // 完成的距离 
    ...
    private float[] x_action = { -1f, 0f, 1f, -1f, 0f, 1f }; // x方向上的动作
    private float[] z_action = { 1f, 1f, 1f, -1f, -1f, -1f }; // z方向上的动作
    private float[] eulerAngles_action = { -1f, 1f }; // 车身欧拉角上的动作
    ...
    {
        if (vectorAction[2] == 1)
        // 恢复状态(根据mcts，这个状态是父节点的状态)
        {

            step_count = (int)(vectorAction[3]);
            // Debug.Log("恢复状:"+step_count);
            // pre_dis = vectorAction[4];
            //pre_steerInput = vectorAction[5];
            this.transform.eulerAngles = new Vector3(0.0f, vectorAction[4], 0.0f);
            this.transform.position = new Vector3(vectorAction[5], 0.0f, vectorAction[6]);
        ...
        else
        {
            ...
            else
            {
                ...
                // 执行动作
                this.transform.position = new Vector3(this.transform.position.x + x_action[(int)vectorAction[0]] * 1.5f, 0.0f, this.transform.position.z + z_action[(int)vectorAction[0]] * 1.5f);
                this.transform.eulerAngles = new Vector3(0.0f, this.transform.eulerAngles.y + eulerAngles_action[(int)vectorAction[1]] * 5f, 0.0f);
                cur_dis = RelativeDis();
                if (this.transform.position.x > 43f ||
                    this.transform.position.x < 35f ||
                    this.transform.position.z > 12f ||
                    this.transform.position.z < 2f ||
                    this.transform.eulerAngles.y < 0f ||
                    this.transform.eulerAngles.y > 45f)
                {
                    AddReward(-2f);
                    Debug.Log("超出限制范围");
                    Done();
                ...
                else
                {
                    ...
                    {
                        AddReward(2f);
                        float relative_eulerAngles_y = this.transform.eulerAngles.y - target_object_eulerAngles_y;

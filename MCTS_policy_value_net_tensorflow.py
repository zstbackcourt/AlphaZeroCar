...
class PolicyValueNet(object):
    ...
    def policy_value_fn(self, action, ob):
        ...
        # print("act_probs:",act_probs[0])

        return zip(action, act_probs[0]), value[0][0]

    def train_step(self, state_batch, mcts_probs, value_batch, lr):
        """perform a training step"""
        _loss, _value_loss,_policy_loss,_l2_penalty, _entropy, _ = self.session.run([self.loss, self.value_loss, self.policy_loss,self.l2_penalty, self.entropy, self.optimizer],
    ...
    def save_model(self):
        # print("保存模型")
        self.saver.save(self.session, self.save_path+"/model")

    ...
    def learn(self, buffer, num_steps, batch_size):
        if buffer.size() <= batch_size:
            ...
            for step in range(num_steps):
                ...
                q_values = self.sess.run(self.q_value_pred, feed_dict={self.state_input: next_state_batch})
                # 计算target q value
                # target_q = np.array(
                #     [data[2] + self.gamma * max_q_values[i] * (1 - data[4]) for i, data in enumerate(minibatch)]
                # )
                target_q = np.array(
                    [data[3] + self.gamma * q_values[i] * (1 - data[4]) for i, data in enumerate(minibatch)]
                )
                #print(target_q)
